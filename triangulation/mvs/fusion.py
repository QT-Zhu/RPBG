...
parser.add_argument('--photo_threshold', type=float, default=0.3)
parser.add_argument('--outdir', help='output dir')
parser.add_argument('--view_num', type=int, help='view number')
...
def read_pair_file(filename, view_num):
    data = []
    with open(filename) as f:
        num_viewpoint = int(f.readline())
        for view_idx in range(num_viewpoint):
            ref_view = int(f.readline().rstrip())
            src_views = [int(x) for i, x in enumerate(f.readline().rstrip().split()[1::2]) if i < view_num-1]
...
def check_geometric_consistency(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src
    ...
    for i in range(2,11):
        ...
        #mask = np.logical_and(dist < 0.7, relative_depth_diff < i/500)
        #mask = np.logical_and(dist < i/2, relative_depth_diff < i/500)
        mask = np.logical_and(dist < i, relative_depth_diff < i/300)
        masks.append(mask)
...
def filter_depth(scan_folder, out_folder, plyfilename, photo_threshold, view_num):
    # the pair file
    ...
    pair_data = read_pair_file(pair_file, view_num)

    for ref_view, src_views in pair_data:
        # load the reference image
        if not os.path.exists(os.path.join(out_folder, 'depth_est/{:0>8}.pfm'.format(ref_view))):
            print("skip", ref_view)
            continue
...
if __name__ == '__main__':
    ...
    filter_depth(scan_folder, out_folder, os.path.join(args.outdir, '../dense_pcd.ply'), photo_threshold, view_num)
