...
def read_model(path):
    ...
    if os.path.exists(os.path.join(path, "points3D.bin")):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dense_folder', help="MVS folder to create")
    parser.add_argument('--image_folder')
    parser.add_argument('--sfm_folder')
    ...
    parser.add_argument('--skip_pair', action='store_true', default=False, help='If set, skip the generation of pair.txt.')

    args = parser.parse_args()

    image_dir = args.image_folder
    model_dir = args.sfm_folder
    cam_dir = os.path.join(args.dense_folder, 'cams')
    renamed_dir = os.path.join(args.dense_folder, 'images')
    os.makedirs(args.dense_folder,exist_ok=True)
    os.makedirs(cam_dir,exist_ok=True)
    os.makedirs(renamed_dir,exist_ok=True)

    cameras, images, points3d = read_model(model_dir)
    num_images = len(list(images.items()))
    print(f"{num_images} images calibrated:")
    valid_IDs = sorted([x for x,_ in images.items()])
    print(valid_IDs)
    ...
    for i in range(num_images):
        idx = valid_IDs[i] # actual image ID
        zs = []
        for p3d_id in images[idx].point3D_ids:
            if p3d_id == -1:
                continue
            transformed = np.matmul(extrinsic[idx], [points3d[p3d_id].xyz[0], points3d[p3d_id].xyz[1], points3d[p3d_id].xyz[2], 1])
        ...
        else: #default depth range
            print(f"image {idx} has no 3D point")
            depth_min = 0.1
            depth_max = 100
        # determine depth number by inverse depth setting, see supplementary material
        if args.max_d == 0:
            image_int = intrinsic[images[idx].camera_id]
            image_ext = extrinsic[idx]
        ...
        depth_ranges[idx] = (depth_min, depth_interval, depth_num, depth_max)
    ...
    if not args.skip_pair:
        ...
        def calc_score(inputs):
            i, j = inputs
            idx_i, idx_j = valid_IDs[i], valid_IDs[j]
            id_i = images[idx_i].point3D_ids
            id_j = images[idx_j].point3D_ids
            id_intersect = [it for it in id_i if it in id_j]
            cam_center_i = -np.matmul(extrinsic[idx_i][:3, :3].transpose(), extrinsic[idx_i][:3, 3:4])[:, 0]
            cam_center_j = -np.matmul(extrinsic[idx_j][:3, :3].transpose(), extrinsic[idx_j][:3, 3:4])[:, 0]
            score = 0
    ...
    for i in range(num_images):
        idx = valid_IDs[i]
        with open(os.path.join(cam_dir, '%08d_cam.txt' % idx), 'w') as f:
            f.write('extrinsic\n')
            for j in range(4):
                for k in range(4):
                    f.write(str(extrinsic[idx][j, k]) + ' ')
                f.write('\n')
            f.write('\nintrinsic\n')
            for j in range(3):
                for k in range(3):
                    f.write(str(intrinsic[images[idx].camera_id][j, k]) + ' ')
                f.write('\n')
            f.write('\n%f %f %f %f\n' % (depth_ranges[idx][0], depth_ranges[idx][1], depth_ranges[idx][2], depth_ranges[idx][3]))
    ...
    if not args.skip_pair:
        with open(os.path.join(args.dense_folder, 'pair.txt'), 'w') as f:
            ...
            for i, sorted_score in enumerate(view_sel):
                idx = valid_IDs[i]
                f.write('%d\n%d ' % (idx, len(sorted_score)))
                for image_id, s in sorted_score:
                    f.write('%d %f ' % (valid_IDs[image_id], s))
                f.write('\n')

    for i in range(num_images):
        idx = valid_IDs[i]
        if args.convert_format:
            img = cv2.imread(os.path.join(image_dir, images[idx].name))
            cv2.imwrite(os.path.join(renamed_dir, '%08d.jpg' % idx), img)
        else:
            shutil.copyfile(os.path.join(image_dir, images[idx].name), os.path.join(renamed_dir, '%08d.jpg' % idx))
