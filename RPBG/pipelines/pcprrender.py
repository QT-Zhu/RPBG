...
class PCPRRender:
    ...
    def update_ds(self, ds_list):
        # basically only need once (one dataset at a time)
    ...
    def render(self, data):
        ...
        total_m = torch.from_numpy(proj_matrix @ np.linalg.inv(view_matrix))
       
        for i,k in enumerate(input_format):
            w = int(self.tgt_sh[0]*(0.5**i))
            h = int(self.tgt_sh[1]*(0.5**i))
            indexs, depths = torch.zeros(len(data['input']['id']),h,w, dtype=torch.int),torch.zeros(len(data['input']['id']),h,w)
